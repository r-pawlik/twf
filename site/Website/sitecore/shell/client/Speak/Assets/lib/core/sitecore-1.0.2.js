...
(function () {
  ...
  _sc.__SITECOREDEBUG = __SITECOREDEBUG;
  _sc.__info = __info;
...
views.View = function (options) {
  ...
  if (options && options.behaviors) {
    _.each(options.behaviors.split(" "), function (behav) {
      self.addBehavior(Sitecore.Behaviors[behav]);
    });
  }
...
_.extend(Sitecore.Definitions.Views.View.prototype, Backbone.View.prototype, {
...
Sitecore.Definitions.App = Backbone.Model.extend({
  appId: undefined,
  modelType: "application",
  initialize: function (options) {
    this.Controls = [];
    if (this.appId) {
      this.localStorage = new Sitecore.Definitions.Data.LocalStorage(this.appId);
    } else {
  ...
  insertRendering: function (itemId, options, cb) {
    ...
    $.get(defaultOptions.path + "?sc_itemid=" + itemId + "&sc_database=" + defaultOptions.database, function (data) {
  ...
  destroy: function () {
    ...
    for (var param in app) {
      if (app[param] instanceof Sitecore.Definitions.App) {
...
Sitecore.Definitions.App.extend = Backbone.View.extend;

_.extend(Sitecore, Backbone.Events);
      ...
      var run = function () {
        if (subdeps.length === 0) {
          ...
          if (pageCodeSrc) {
            ...
            if (firstLoad) {
              app = Sitecore.Factories.createApp();
            } else {
              app = Sitecore.Factories.createApp(scopedEL.attr("id"), scopedEL.attr("id"));
            }
            if (firstLoad) {
      ...
      addItemSelectorUrlPortion: function(url, itemSelector, options) {
        if (itemSelector && _.isString(itemSelector)) {
          ...
          if (itemSelector.indexOf("search:") === 0) {
            ...
            if (options && options.root && Sitecore.Helpers.id.isId(options.root)) {
...
var LayoutManager = Sitecore.Definitions.Views.View.extend({
...
Sitecore.Definitions.Views.CompositeView = Backbone.Layout = Backbone.LayoutView = Backbone.LayoutManager = LayoutManager;
// Tack on the version.
...
_.extend(data.Database.prototype, {
  ...
  getUrl: function(itemSelector, options) {
    ...
    var url = Sitecore.Web.itemWebApi.getUrl(itemSelector, options);
    ...
    var updateFields = function (data) {
      ...
      _.each(itemData.Fields, function (field, fieldId) {
        ...
        if (f != null) {
          ...
          this.$fields.push(this, new Sitecore.Definitions.Data.Field(fieldUri, field.Name, field.Value, field.Type));
        }
      }, this);
...
var ItemBackbone = Sitecore.Definitions.Models.Model.extend({
  idAttribute: "itemId",
  getUrl: function () {

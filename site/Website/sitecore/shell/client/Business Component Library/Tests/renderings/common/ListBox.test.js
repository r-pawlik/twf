        require(["jasmineEnv", "/-/speak/v1/business/listbox.js"], function (jasmineEnv) {
          ...
          var setupTests = function (testAreaEl) {
            var descriptor = {
              attributes: [
                ...
                { name: "valueFieldName", defaultValue: null, which: "which holds the the name of the data field used as ListBox option value" },
                { name: "displayFieldName", defaultValue: null, which: "which holds the the name of the data field used as ListBox option text" },               
                { name: "selectedValues", defaultValue: [], which: "which holds the values array of selected items" },
                { name: "selectedItemIds", defaultValue: [], which: "which holds the ids of the selected items" }
              ]};

            describe("Given a ListBox model", function () {
              var model;
              var $element;
              var items = [
                {
              ...
              it("it should create the Control when I execute Run", function () {
                $element = testAreaEl.find(".sc-listbox");
                var testArea = _sc.Factories.createApp(testAreaEl.attr("id"));
                expect(testArea.ListBox2).toBeDefined();
                model = testArea.ListBox2;
              });
              ...
              it("it should not have a selectedItem", function () {
                expect(model.get("selectedItem")).toBe(null);
              });

              it("it should have a selectedValues defined as an array of itemIds when we set selectedItems = [items[0], items[1]]", function () {  
                model.set("selectedItems", [items[0], items[1]]);
                expect(model.get("selectedValues").toString()).toBe([items[0].itemId, items[1].itemId].toString());
              });
              ...
              it("it should change the options collection, when we call rebind(items2)", function () {
          ...
          runTests(jasmineEnv, setupTests, "ListBox.htm");
        });

...
//      });
      ...
      it("it should add a disabled class to the list item if isEnabled is false using the disable method", function () {
        var firstActionElement = $e.find(".sc-actionMenu-item").eq(0),
          firstAction = m.viewModel.getAction(firstActionElement);

        firstAction.isEnabled(true);
        expect(firstActionElement.hasClass("disabled")).toBe(false);

        firstAction.disable();
        expect(firstActionElement.hasClass("disabled")).toBe(true);
      });
      
      it("it should remove the disabled class from the list item if isEnabled is true using the enable method", function () {
        var firstActionElement = $e.find(".sc-actionMenu-item").eq(0),
          firstAction = m.viewModel.getAction(firstActionElement);
    ...
    describe("Given an Action control inside appropriate application", function () {
      ...
      it("it should have the 'actionsStatus' array with 2 actions item set to favurite", function () {
        expect(m.get("actionsStatus").length).toBeDefined();
        expect(m.get("actionsStatus").length).toBe(2);
      });

      it("when a 3rd action is added to favorite, it should have the 'actionsStatus' array with 3 actions", function () {
        var favIcon = $e.find(".favorite-icon").eq(3);
        expect(favIcon.hasClass("selected")).toBe(false);
        favIcon.trigger("click");
        expect(m.get("actionsStatus").length).toBe(3);
      });
...
//      });
      ...
      describe("Given an Action model from the ActionControl", function () {
        ...
        it("it should have the isEnabled property to be defined", function () {
          expect(action.isEnabled).toBeDefined();
        });
        it("it should have the isEnabled property to be true", function () {
          expect(action.isEnabled()).toBe(true);
        });
        it("it should have the isEnabled property to be false if the data-sc-isdisabled is true", function () {
          action = m.get("actions")[1];
          var $action = m.viewModel.$el.find('[data-sc-actionid="' + action.id() + '"]');
          expect($action.attr("data-sc-isdisabled") === "true").toBe(true);
          expect(action.isEnabled()).toBe(false);
        });

        
        it("it should have the disable property to be defined as a function", function () {
          expect(action.disable).toBeDefined();
          expect(typeof action.disable).toBe("function");
        });
        
        it("it should have the enable property to be defined as a function", function () {
          expect(action.enable).toBeDefined();
          expect(typeof action.enable).toBe("function");
        });
        
        it("it should set isEnabled to true when enable method is executed", function () {
        ...
        it("it should set isEnabled to false when disable method is executed", function () {
